...
const COINGECKO_TOKEN_PRICE_URL = (token) =>
  `https://api.coingecko.com/api/v3/simple/token_price/ethereum?vs_currencies=usd&contract_addresses=${token}`;
...
bondGraph(SUBGRAPH_URL, {
  ...
  const getTokenPrice = async (element) => {
    ...
    const tokenPrice = await fetch(COINGECKO_TOKEN_PRICE_URL(collateralToken));
